...
const LOAD_MSGS = [
  ...
  'telling the pixels what to do...',
  'summoning the art gods...',
  'running on pure vibes rn...',
  'making something questionable...',
  'GPU go brrr...',
  'adding secret sauce...',
...
export default function ImagePage() {
  ...
  const enhance = async () => {
    ...
    try {
      const result = await chat(
        [{ role: 'user', content: `Enhance this image prompt to be more vivid and detailed. Under 200 words. Output only the enhanced prompt, nothing else.\n\nOriginal: "${prompt}"` }],
        'mistral', ''
      )
      setEnhanced(result)
  ...
  const handlePreset = (p) => {
    ...
    if (p.id !== preset) { setRatio(p.ratio); setStyle(p.style) }
  }
      ...
      {(tab === 'generate' || tab === 'creative') && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '8px', marginBottom: '12px' }}>
            ...
            <div>
              <label style={{ fontSize: '0.75rem', color: 'var(--fg3)', display: 'block', marginBottom: '4px' }}>count</label>
              <input type="number" min={1} max={5} value={count} onChange={e => setCount(Math.min(5, Math.max(1, +e.target.value)))} />
            </div>
          </div>

          <div style={{ position: 'relative', marginBottom: '8px' }}>
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="describe what you want to see..." rows={3} style={{ paddingRight: '80px' }} />
            <button
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '40px', color: 'var(--fg3)' }}>
              <div style={{ fontSize: '2rem', marginBottom: '8px', animation: 'blink 1s infinite' }}>🎨</div>
