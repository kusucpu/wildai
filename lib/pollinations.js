...
export async function generateImage(prompt, opts = {}) {
  ...
  const byopKey = getByopKey()
  if (!byopKey) throw new Error('premium_no_key')

  const res = await fetch('/api/image', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-byop-key': byopKey },
...
export const FREE_CHAT_MODELS = [
  ...
  { id: 'llama-scout',  name: 'Meta',    note: 'cheapest, safe' },
  ...
  { id: 'polly',         name: 'Polly',     note: 'real-time' },
